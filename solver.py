...
def cosine_similarity_matrix(pe):
    pe_norm = pe / pe.norm(dim=-1, keepdim=True)
    cos_sim = torch.matmul(pe_norm, pe_norm.transpose(-1, -2))
    cos_sim = (cos_sim + 1.0) / 2.0
    return cos_sim

def pe_loss_i_k_matrix(cos_sim):
    B, L, _ = cos_sim.size()
    device = "cuda:0" if torch.cuda.is_available() else "cpu"

    epsilon = torch.tensor(1 / (L), device=device)
    i_grid = torch.arange(L-1, device=device).view(1, L-1, 1)
    k_grid = torch.arange(-L+2, L, device=device).view(1, 1, 2*L-2)

    mask = (
            ((k_grid >= 2) & (k_grid <= (L - i_grid - 1))) |
            ((k_grid <= -2) & (k_grid >= -i_grid))
    )
    mask = mask.expand(B, L - 1, 2*L-2)

    batch_indices = torch.arange(B, device=device).view(B,1,1).expand(B, L-1, 2*L-2)
    
    idx_i_k = torch.clamp(i_grid + k_grid, 0, L - 1)
    
    loss_matrix = torch.square(cos_sim[batch_indices, i_grid, i_grid + 1] - cos_sim[batch_indices, i_grid, idx_i_k] - epsilon * (torch.abs(k_grid - 1)).expand(B,L-1,2*L-2)) * mask
    return loss_matrix

def pe_loss(pe_loss_i_k):
    valid_mask = (pe_loss_i_k != 0)
    valid_values = pe_loss_i_k[valid_mask]

    if valid_values.numel() == 0:
        return torch.tensor(0.0, device=pe_loss_i_k.device)
...
class Solver(object):
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            pe = self.model.embedding.position_embedding(input)
            cos_sim = cosine_similarity_matrix(pe)
            pe_loss_i_k = pe_loss_i_k_matrix(cos_sim)
            batch_pe_loss = pe_loss(pe_loss_i_k)

            rec_loss = self.criterion(output, input)

            loss_1.append((rec_loss + batch_pe_loss - self.k * series_loss).item())
            loss_2.append((rec_loss + batch_pe_loss + self.k * prior_loss).item())
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                batch_pe_loss = pe_loss(pe_loss_i_k)
                rec_loss = self.criterion(output, input)
                loss1_list.append((rec_loss + batch_pe_loss - self.k * series_loss).item())
                loss2_list.append((rec_loss + batch_pe_loss + self.k * series_loss).item())
                loss1 = rec_loss + batch_pe_loss - self.k * series_loss 
                loss2 = rec_loss + batch_pe_loss + self.k * prior_loss
            ...
            total_train_loss = train_loss + train_loss2

            print("Epoch: {0} finished | Avg Train Loss1: {1:.7f} | Avg Train Loss2: {2:.7f} | Total Avg Loss: {3:.7f}".format(
                epoch + 1, train_loss, train_loss2, total_train_loss))
